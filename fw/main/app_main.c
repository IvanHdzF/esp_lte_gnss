...
static void mqtt5_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_SUBSCRIBED:
        ...
        ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
        //msg_id = esp_mqtt_client_publish(client, "sensors/gnss", "data", 0, 0, 0);
        ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
        break;
...
void process_gnss_data(const void* data, uint16_t len)
{
    if (mqtt_client) {
        int msg_id = esp_mqtt_client_publish(mqtt_client, "sensors/gnss", data, len, 0, 0);
        ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
    }
}
...
void app_main(void)
{
    ...
    gnss_set_callback(process_gnss_data);
    gnss_init();
